<template>
  <leechat-modal :is-open="isOpen">
    <div class="modify-user-info" @click.stop>
      <leechat-avatar :info="{ name: name, avatar: avatar }" size="70px" />
      <leechat-input
      ...
      <div class="flex gap-[15px] w-full justify-end mt-[20px]">
        <leechat-button type="minor" @click="isOpen = false">取消</leechat-button>
        <leechat-button @click="onUpdateUser">确定</leechat-button>
      </div>
    </div>
  </leechat-modal>
</template>
<script setup>
import LeechatModal from '@/components/LeechatModal.vue'
import LeechatAvatar from '@/components/LeechatAvatar.vue'
import LeechatInput from '@/components/LeechatInput.vue'
import LeechatButton from '@/components/LeechatButton.vue'
